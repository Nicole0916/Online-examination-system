...
public class ExamController {
	...
	public Map<String, String> add(Exam exam){
		...
		//获取填空题总数
		queryMap.put("questionType", Long.valueOf(Question.QUESTION_TYPE_FILL));
		int fillQuestionTotalNum = questionService.getQuestionNumByType(queryMap);
		if(exam.getFillQuestionNum() > fillQuestionTotalNum){
			ret.put("type", "error");
			ret.put("msg", "填空题数量超过题库判断题总数，请修改!");
			return ret;
		}
		//获取简答题总数
		queryMap.put("questionType", Long.valueOf(Question.QUESTION_TYPE_SHORT));
		int shortQuestionTotalNum = questionService.getQuestionNumByType(queryMap);
		if(exam.getShortQuestionNum() > shortQuestionTotalNum){
			ret.put("type", "error");
			ret.put("msg", "简答题数量超过题库判断题总数，请修改!");
			return ret;
		}
		
		
		exam.setQuestionNum(exam.getSingleQuestionNum() + exam.getMuiltQuestionNum() + exam.getChargeQuestionNum() + exam.getFillQuestionNum() + exam.getShortQuestionNum());
		exam.setTotalScore(exam.getSingleQuestionNum() * Question.QUESTION_TYPE_SINGLE_SCORE + exam.getMuiltQuestionNum() * Question.QUESTION_TYPE_MUILT_SCORE + exam.getChargeQuestionNum() * Question.QUESTION_TYPE_CHARGE_SCORE + exam.getFillQuestionNum()*Question.QUESTION_TYPE_FILL_SCORE + exam.getShortQuestionNum()*Question.QUESTION_TYPE_SHORT_SCORE);
	...
	public Map<String, String> edit(Exam exam){
		...
		if(exam.getFillQuestionNum() >fillQuestionTotalNum){
			ret.put("type", "error");
			ret.put("msg", "判断题数量超过题库填空题总数，请修改!");
			return ret;
		}
		//获取简答题总数
		queryMap.put("questionType", Long.valueOf(Question.QUESTION_TYPE_SHORT));
